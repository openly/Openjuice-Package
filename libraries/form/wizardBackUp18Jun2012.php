...
class OJWizard extends Renderable{
	...
	public function getForm(){
		$frm = new OJForm($this->currentStepFields,
			...
			$this->formRules[$this->currentStepName],$this->fieldGroups);
	...
	public function hasNextStep(){
		return $this->currentStepIndex < ($this->_count - 1);
	}
	...
	public function canFinishAtThisStep(){
		...
		$reload = false;
		//var_dump($this->_stepValidations);
		if($this->hasNextStep())
		{
			for($i = 0; $i<$this->_count;$i++)
			{
				if(!isset($this->_stepValidations[$i]))
				{
					...
					echo "$i - {$this->_stepValidations[$i]}<br/>"; 
					$reload = true;
				}
